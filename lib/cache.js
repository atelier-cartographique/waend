...
function Layer () {
    this.parameters = ['id', 'user_id', 'type', 'properties'];
};
...
function Composition () {
    this.parameters = ['id', 'layer_id', 'group_id', 'order'];
};
util.inherits(Layer, AbstractObject);
...
_.extend(Cache.prototype, {
    ...
    _initCacheCallback: function (type, results) {
        ...
        for (var i = results.length - 1; i >= 0; i--) {
            ...
            self._insert(type, obj, 
                function(){
                    console.log('Cache._initCacheCallback', type, obj.id);
            }, function(err){
    ...
    indexMap: function(group) {
        ...
        this.mapIndex.index(group.id, text);
    },

    indexAuth: function (user) {
        this.indexAuth[user.auth_id] = user.id;
    },
    ...
    getLayers: function (groupId) {
        ...
        return Promise.resolve(this.groupIndex[groupId]);
    },

    getMaps: function (expr) {
        var maps = this.mapIndex.search(expr);
        return Promise.resolve(maps);
    },

    getAuthenticatedUser: function (auth_id) {
        if(!(auth_id in this.indexAuth)){
            return Promise.reject(new CacheError('auth_id ['+auth_id+'] not indexed'));
        }
        return this.get('user', this.indexAuth[auth_id]);
    }
});

...
CacheStore.prototype.get = function (gid) {
    if (!(gid in this.groups)) {
        ...
        return Promise.resolve(this.groups[gid]);
    }
};
...
CacheStore.prototype.updateGroups = function (objType, data) {
    ...
    else if ('layer' === objType) {
        if (data.id in deps) {
            groups = _.clone(deps[data.id]);
        }
    }
    else if ('entity' === objType
             || 'path' === objType
             || 'spread' === objType) {
        if (data.layer_id in deps) {
            groups = _.clone(deps[data.layer_id]);
        }
    }
    ...
    _.each(groups, function(gid){
        this.get(gid)
            .then(function(item){
                item.loadFromPersistent()
                    .then(this.updateDeps.bind(this));
            })
            .catch(logError('CacheStore.updateDeps'));
    }, this);
    ...
    delFeature: function (lid, id) {
        ...
        var removeFromStorage = function (feature, lid, id, resolve, reject) {
            ...
            self.db.query(queryName, [id])
                .then(function(){
                    resolve();
                    self.cs.updateGroups(featureType, feature);
                })
                .catch(reject);
        };
